...
class Client(object):
    ...
    def chk_conn(self, status_path='status'):
        if self.base_url is None:
            raise KongAdmConfigurationError('kongadm URL not found.')
        try:
            body = self._execute('GET', status_path)
    ...
    def execute(self, http_method, path, params=None, content_type='application/json'):
        if self.chk_conn():
            return self._execute(http_method, path, params, content_type)
        return None

    def _execute(self, http_method, path, params=None, content_type='application/json'):
